...
parser.add_argument('--config', default='./configs/ecssd_rkhs.yaml', type=str) #default='./configs/plain_rkhs.yaml'
parser.add_argument('--mode', default=None, type=str)
parser.add_argument('--save_path', default='./checkpoints/ecssdplus_rkhs', type=str) #default='./checkpoints/rkhs'
...
def main():
    ...
    while not stop_training:
        ...
        for i, (img, mask, edge, prob, cls_label, id) in enumerate(trainloader):
            ...
            if cfg['fk_form'][:2] == 'ce':
                data_loss = loss_calc(pred.squeeze(1), prob,
            ...
            # total loss
            loss = data_loss + cfg['w_e_alpha'] * weight_edge_loss
            ...
            if epoch != 0 and (i%100)==0:
                logger.info('Iter:{:}, avg loss (total): {:.6f}'.format(i, loss_m.avg)
                            + ' | (data term): {:.6f} | (edge term): {:.6f}'.format(data_m.avg, weight_edge_m.avg)
                            + ' | ratio: {:.6f}'.format(weight_edge_m.avg / (data_m.avg + 1e-6)))
            if epoch == 0 and (i%50)==0:
                logger.info('Iter:{:}, image loss (total): {:.6f}'.format(i, loss)
                            + ' | (data term): {:.6f} | (edge term): {:.6f}'.format(data_loss, weight_edge_loss)
