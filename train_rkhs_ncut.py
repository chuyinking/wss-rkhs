...
parser.add_argument('--config', default='./configs/ecssd_ncut.yaml', type=str) #default='./configs/plain_rkhs.yaml'
parser.add_argument('--mode', default=None, type=str)
parser.add_argument('--save_path', default='./checkpoints/ecssdplus_ncut', type=str) #default='./checkpoints/rkhs'
...
def main():
    ...
    while not stop_training:
        ...
        for i, (img, mask, edge, prob, cls_label, id) in enumerate(trainloader):
            ...
            kcut_loss = weightEdge(pred, edge, beta=0, kernel_size=3, filter_mode=cfg['kcut_mode'],
                                          weight_mode='raw')
            ncut_loss = Ncut(pred, filter_mode='Bilateral', kernel_size=3)

            # total loss
            loss = data_loss + cfg['w_e_alpha'] * kcut_loss + cfg['w_ncut_alpha'] * ncut_loss
            ...
            kcut_m.update(kcut_loss.item(), img.size()[0])
            ncut_m.update(ncut_loss.item(), img.size()[0])
            ...
            if epoch != 0 and (i%100)==0:
                logger.info('Iter:{:}, avg loss (total): {:.6f}'.format(i, loss_m.avg)
                            + ' | (scribble term): {:.6f} | (kcut term): {:.6f} | (ncut term): {:.6f}'.format(data_m.avg, kcut_m.avg, ncut_m.avg))
            if epoch == 0 and (i%50)==0:
                logger.info('Iter:{:}, image loss (total): {:.6f}'.format(i, loss)
                            + ' | (scribble term): {:.6f} | (kcut term): {:.6f} | (ncut term): {:.6f}'.format(data_loss, kcut_m.avg, ncut_loss))
        ...
        loss_logging = 'Epoch {} | loss:{:.3f}, scribble_loss:{:.3f}, kcut_loss:{:.3f}, ncut_loss:{:.3f} ' \
                       '| mIOU:{:.2f}\n'.format(epoch, loss_m.avg,
                                                data_m.avg, kcut_m.avg,
                                                ncut_m.avg, mIOU)
        records.add_batch(loss_m.avg, data_m.avg, kcut_m.avg, mIOU, mDICE, mPA)
