...
parser.add_argument('--config', default='./configs/ecssd_rkhs_pce.yaml', type=str) #default='./configs/plain_rkhs.yaml'
parser.add_argument('--mode', default=None, type=str)
parser.add_argument('--save_path', default='./checkpoints/ecssdplus_rkhs_pce', type=str) #default='./checkpoints/rkhs'
...
def main():
    ...
    while not stop_training:
        ...
        for i, (img, mask, edge, prob, cls_label, id) in enumerate(trainloader):
            ...
            pce_loss = loss_calc(pred.squeeze(1), prob,
                                 ignore_index=255, multi=False,
                                 class_weight=False, ohem=False)

            data_loss = fidelityloss(pred, prob, fk_form=cfg['fk_form'], n_class=cfg['nclass'])

            weight_edge_loss = weightEdge(pred, edge, beta=cfg['w_e_Beta'], kernel_size=3, filter_mode='Gaussian',
                                          weight_mode=cfg['w_e_type'])

            # total loss
            loss = pce_loss + cfg['rkhs_alpha'] * (data_loss + cfg['w_e_alpha'] * weight_edge_loss)
            ...
            pce_m.update(pce_loss.item(), img.size()[0])
            weight_edge_m.update(weight_edge_loss.item(), img.size()[0])
            ...
            if epoch != 0 and (i%100)==0:
                logger.info('Iter:{:}, avg loss (total): {:.6f}'.format(i, loss_m.avg)
                            + ' | (pce term): {:.6f} | (data term): {:.6f} | (edge term): {:.6f}'.format(pce_m.avg, data_m.avg, weight_edge_m.avg)
                            + ' | ratio: {:.6f}'.format(weight_edge_m.avg / (data_m.avg + 1e-6)))
            if epoch == 0 and (i%50)==0:
                logger.info('Iter:{:}, image loss (total): {:.6f}'.format(i, loss)
                            + ' | (pce term): {:.6f} | (data term): {:.6f} | (edge term): {:.6f}'.format(pce_m.avg, data_m.avg, weight_edge_m.avg)
                            + ' | ratio: {:.6f}'.format(weight_edge_loss/data_loss))

        mIOU, mDICE, mPA = evaluate(model, valloader, 'original', cfg)[0:3]
        #scores = evaluate(model, valloader, eval_mode, cfg)[-1]

        logger.info('***** Evaluation {} ***** >>>> mIOU: {:.2f}, mDICE:{:.2f}, mPA:{:.2f}'.format('original', mIOU, mDICE, mPA))
        loss_logging = 'Epoch {} | loss:{:.3f}, data_loss:{:.3f}, edge_loss:{:.3f} ' \
                       '| mIOU:{:.2f}\n'.format(epoch, loss_m.avg,
                                                                data_m.avg,
                                                                weight_edge_m.avg, mIOU)
        records.add_batch(loss_m.avg, data_m.avg, weight_edge_m.avg, mIOU, mDICE, mPA)
